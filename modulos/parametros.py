...
def aplicarNivel(self):
    ...
    NumberOfSequences = "Application:Sequencing int NumberOfSequences= " + \
        str(nivel) + " 15 1 % // number of sequences in a set of intensifications\n"
    EpochsToAverage = "Filtering:P3TemporalFilter int EpochsToAverage= " + \
        str(nivel) + " 1 0 % // Number of epochs to average"
...
def aplicarSecuenciaTerapia(self):
    ...
    lista = ("A A 1 ", "B B 1 ", "C C 1 ", "D D 1 ", "E E 1 ", "F F 1 ",
             "G G 1 ", "H H 1 ", "I I 1 ")  # necesario para construir el archivo prm
    random.shuffle(orden_secuencia)
    for i in range(0, 9):

        if orden_secuencia[i] != 0:
            orden_img = lista[i] + img_path + "/img" + \
                str(orden_secuencia[i]) + ".png % % "
        else:
            orden_img = lista[i] + install_path + "/img" + \
                "/img" + str(orden_secuencia[i]) + ".png % % "
...
def aplicarSecuenciaCalibracion(self):
    ...
    if self.calibracion_tarea == 1:
        ...
        text_to_spell = "Application:Speller string TextToSpell= " + \
            constantes.TAREA_UNO + " // character or string to spell in offline copy mode"
        ...
        text_to_spell = "Application:Speller string TextToSpell= " + \
            constantes.TAREA_DOS + " // character or string to spell in offline copy mode"
        ...
        text_to_spell = "Application:Speller string TextToSpell= " + \
            constantes.TAREA_TRES + " // character or string to spell in offline copy mode"
    ...
    QtCore.QCoreApplication.processEvents()
    for i in range(0, 9):
        orden_img = lista[i] + ubicacion_img + \
            "/img" + str(orden_sec[i]) + ".png % % "
        fout.write(orden_img)
